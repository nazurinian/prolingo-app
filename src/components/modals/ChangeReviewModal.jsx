        ...
        {!isCsvDirty ? (
          ...
          <>
            {csvChangeSummary.addedItems.length > 0 && <div>
              ...
              <div className="space-y-1">{csvChangeSummary.addedItems.map(item => <div key={`add-${item.id}`} className="flex items-center gap-2 p-2 rounded-lg border border-emerald-100 dark:border-emerald-900 bg-emerald-50/50 dark:bg-emerald-900/10"><span className="text-[10px] font-mono text-slate-400 w-12">#{item.displayId}</span><div className="min-w-0 flex-1"><p className="text-xs font-bold text-slate-700 dark:text-slate-200 truncate">{capitalizeDisplayText(item.word)}</p><p className="text-[9px] font-mono text-slate-400 truncate">{item.vocabId || item.id}</p></div><button onClick={() => applyChangeRevert(item.id, 'added')} className="px-2 py-1 text-[10px] rounded border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300">Remove</button></div>)}</div>
            </div>}
            {csvChangeSummary.modifiedItems.length > 0 && <div>
              ...
              <div className="space-y-1">{csvChangeSummary.modifiedItems.map(change => <div key={`mod-${change.after.id}`} className="flex items-center gap-2 p-2 rounded-lg border border-amber-100 dark:border-amber-900 bg-amber-50/50 dark:bg-amber-900/10"><span className="text-[10px] font-mono text-slate-400 w-12">#{change.after.displayId}</span><div className="min-w-0 flex-1"><p className="text-xs font-bold text-slate-700 dark:text-slate-200 truncate">{capitalizeDisplayText(change.after.word)}</p><p className="text-[9px] text-slate-400 truncate">from “{capitalizeDisplayText(change.before.word)}” • {change.after.vocabId || change.after.id}</p></div><button onClick={() => applyChangeRevert(change.after.id, 'modified')} className="px-2 py-1 text-[10px] rounded border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300">Revert</button></div>)}</div>
            </div>}
            {csvChangeSummary.deletedItems.length > 0 && <div>
              ...
              <div className="space-y-1">{csvChangeSummary.deletedItems.map(item => <div key={`del-${item.id}`} className="flex items-center gap-2 p-2 rounded-lg border border-red-100 dark:border-red-900 bg-red-50/50 dark:bg-red-900/10"><span className="text-[10px] font-mono text-slate-400 w-12">#{item.displayId}</span><div className="min-w-0 flex-1"><p className="text-xs font-bold text-slate-700 dark:text-slate-200 truncate">{capitalizeDisplayText(item.word)}</p><p className="text-[9px] font-mono text-slate-400 truncate">{item.vocabId || item.id}</p></div><button onClick={() => applyChangeRevert(item.id, 'deleted')} className="px-2 py-1 text-[10px] rounded border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400">Restore</button></div>)}</div>
            </div>}
          </>
        )}
      </div>
