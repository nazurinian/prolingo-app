...
export const MemoizedRow = memo(({
    ...
    const destructiveActionsLocked = Boolean(isSystemBusy);
            ...
            >
                ...
                <div className="flex flex-col md:flex-row justify-between items-start gap-1.5 md:gap-2 h-full">
                    ...
                    <div className={`hidden md:flex md:flex-col md:ml-2 justify-center items-end w-[78px] gap-1.5 flex-shrink-0 md:border-l md:pl-2 ${isActive ? 'border-blue-500' : 'border-slate-100 dark:border-slate-700'}`}>
                        ...
                        <div className="flex gap-1 w-full"><button disabled={destructiveActionsLocked} onClick={(e) => { e.stopPropagation(); if (destructiveActionsLocked) return; onEditItem(item); }} className={`flex-1 h-[24px] flex items-center justify-center rounded border border-blue-100 dark:border-blue-900 text-blue-500 ${destructiveActionsLocked ? 'opacity-35 cursor-not-allowed' : ''}`} title={destructiveActionsLocked ? "Edit locked while audio task is running" : "Edit vocabulary"}><Edit3 className="w-3 h-3"/></button><button disabled={destructiveActionsLocked} onClick={(e) => { e.stopPropagation(); if (destructiveActionsLocked) return; onDeleteItem(item); }} className={`flex-1 h-[24px] flex items-center justify-center rounded border border-red-100 dark:border-red-900 text-red-500 ${destructiveActionsLocked ? 'opacity-35 cursor-not-allowed' : ''}`} title={destructiveActionsLocked ? "Delete locked while audio task is running" : "Delete vocabulary"}><Trash2 className="w-3 h-3"/></button></div>
                ...
                <>
                    ...
                    <section className="pointer-events-auto relative z-10 w-full max-w-sm rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-2xl p-3" style={{ maxHeight: `calc(var(--prolingo-overlay-height, 100dvh) - ${MOBILE_BOTTOM_PLAYER_RESERVE + 28}px - env(safe-area-inset-bottom, 0px))` }}>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            <button disabled={destructiveActionsLocked} onClick={(e) => { e.stopPropagation(); if (destructiveActionsLocked) return; onEditItem(item); onMenuToggle(null); }} className={`min-h-10 px-3 rounded-xl flex items-center justify-center gap-2 text-[10px] font-bold border border-blue-100 dark:border-blue-900 text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 ${destructiveActionsLocked ? 'opacity-35 cursor-not-allowed' : ''}`}><Edit3 className="w-3.5 h-3.5"/> Edit</button>
                            <button disabled={destructiveActionsLocked} onClick={(e) => { e.stopPropagation(); if (destructiveActionsLocked) return; onDeleteItem(item); onMenuToggle(null); }} className={`min-h-10 px-3 rounded-xl flex items-center justify-center gap-2 text-[10px] font-bold border border-red-100 dark:border-red-900 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 ${destructiveActionsLocked ? 'opacity-35 cursor-not-allowed' : ''}`}><Trash2 className="w-3.5 h-3.5"/> Delete</button>
