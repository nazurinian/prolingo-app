...
import { Upload, Plus, FileDown, Trash2, History, RotateCcw, Layers } from 'lucide-react';
...
export default function DesktopDataActions({
  ...
  undoStack, undoLastDataChange, saveUpdatedCSV, isMultiSourceMode, activeTextEditorModel, isBatchOpen = false, setIsBatchOpen, isBatchDownloading = false
}) {
  return (
                  ...
                  <>
                   ...
                   <>
                       <div className="col-span-2 mb-1">
                          <p className="text-[10px] text-slate-400 text-center border dark:border-slate-700 p-1.5 rounded bg-slate-50 dark:bg-slate-800">{activeTextEditorModel === 'legacy-line-v1' ? 'DATA = Library / JSON / CRUD / Backup + Batch. AUDIO = voice, speed, local audio, download defaults & sources.' : 'DATA = Library / JSON / CRUD / Backup + Batch. AUDIO = voice, speed, local audio, download defaults & sources.'}</p>
                       </div>
                       <button type="button" disabled={isSystemBusy && !isBatchDownloading} onClick={() => setIsBatchOpen?.(!isBatchOpen)} className={`col-span-2 flex items-center justify-center gap-2 p-2 rounded text-xs font-black border ${isBatchOpen ? 'bg-purple-600 border-purple-600 text-white' : 'bg-purple-50 dark:bg-purple-950/20 border-purple-200 dark:border-purple-800 text-purple-700 dark:text-purple-300'} disabled:opacity-50`}><Layers className="w-3.5 h-3.5"/>{isBatchDownloading ? 'BATCH • RUNNING' : 'BATCH AUDIO'}</button>
