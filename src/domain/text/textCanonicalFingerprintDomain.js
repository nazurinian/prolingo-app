...
export const canonicalJsonStringify = value => JSON.stringify(canonicalizeJsonValue(value));

const utf8Bytes = value => {
  if (typeof TextEncoder !== 'undefined') return new TextEncoder().encode(String(value ?? ''));
  const encoded = unescape(encodeURIComponent(String(value ?? ''))); // legacy fallback
...
export const buildCanonicalSha256Fingerprint = (value, prefix = 'sha256') => `${clean(prefix) || 'sha256'}-${sha256Hex(canonicalJsonStringify(value))}`;
